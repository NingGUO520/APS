...
and appli_func f list_v env memoire= 
	match f with
	|InF(e,args,list_value)-> let func_env = appli_args args list_v in 
					eval_expr e (func_env@env) memoire
...
|ASTId id -> let v = getValeur id env memoire in 
		 (	match v with
			|InA(n)-> let value = List.assoc v memoire in InN(value)   (*ID1*)
			|_ -> v 	(*ID2*)    
		 )
|ASTOPrim(op,e1,e2)-> 
...
|ASTFuncExpr(args,e)->let l_args = getArgs args in InF(e,l_args,(init_list_value l_args))
...
|Var(id,letype) -> adresse := !adresse + 1 ; 
					let m = alloc memoire !adresse in  
...
let eval_stat stat env memoire w =
	...
	|IfProc(b ,bloc1,bloc2)-> let v = get_int(eval_expr b env memoire) in 
							...
							else if v1 == 0 then eval_bloc bloc2 env memoire w 
							else failwith "évaluation if erreur"
	
	|ASTWhile(b, bloc)-> let v = get_int(eval_expr b env memoire) in 
						if v == 0 then memoire,w 
						else if v == 1 then let res_mem, res_w = eval_bloc bloc env memoire w in
...
and eval_bloc bk env memoire w = 
	let res_env, res_w = eval_cmds bk env w in res_env, res_w
